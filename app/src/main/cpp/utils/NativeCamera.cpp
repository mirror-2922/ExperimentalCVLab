...
extern InferenceEngine* get_active_engine();
...
extern float safeYoloDetection(cv::Mat& frame);
...
bool NativeCamera::open(int facing, int width, int height, jobject vSurface, jobject mSurface) {
    ...
    AImageReader_new(1280, 720, AIMAGE_FORMAT_YUV_420_888, 4, &image_reader);
...

    {
        ...
        if (vSurface) viewfinder_window = ANativeWindow_fromSurface(getJNIEnv(), vSurface);
...
void NativeCamera::close() {
    ...
    if (capture_session) {
        ...
        ACameraCaptureSession_abortCaptures(capture_session);
...
bool NativeCamera::fetch_latest_frame(int* pixels, int w, int h) {
    std::lock_guard<std::mutex> lock(frame_mutex);
    if (latest_frame.empty()) return false;
    cv::Mat resized;
    cv::resize(latest_frame, resized, cv::Size(w, h));
    memcpy(pixels, resized.data, w * h * 4);
    return true;
}
...
void NativeCamera::onImageAvailable(void* context, AImageReader* reader) {
    ...
    // Preprocessing (Rotate then Resize-then-Crop)
    cv::Mat rotated;
    ...
    // AI composite
    float latency = safeYoloDetection(cropped);
    ...
    // Cache for Kotlin getLatestFrame
    {
        std::lock_guard<std::mutex> lock(self->frame_mutex);
        cv::cvtColor(cropped, self->latest_frame, cv::COLOR_RGBA2BGRA);
    }
